...
//#include <protocols/abinitio/LJcore.fwd.hh>
...
///@brief Populatio initialization ==================================================================================================================
void
ClassicAbinitio::generate_population(){
	...
	cout << "###========================== CGLFold_GPCR: Generating the initial population =======================" << endl;
...
///=======================================用户添加程序=====================================================================
			for (Size i = 0; i < NP_; ++i){
			    ...
			    Initial_population.push_back(new_pose);
			}
///======================================================================================================================
...
///=======================================用户添加程序===============================stage2================================
			for (Size i = 0; i < Initial_population.size(); ++i){
				mc_->reset( Initial_population[i] );
				success = do_stage2_cycles( Initial_population[i] );
				recover_low( Initial_population[i], STAGE_2 );
			}
///======================================================================================================================                 
...
///===== @BEGIN =========== @BEGIN ============ @BEGIN ============ @BEGIN ============ @BEGIN ============
	///@brief create output dirctory
//	if ( !access("output_files", 00) )
	//	system("rm -r output_files");
//	system("mkdir -p output_files/SPICKER_data");
//	system("mkdir -p output_files/PDB_population");
//	system("mkdir -p output_files/Experiment_of_only_FA");
//	SPICKER_All_Data.open("./output_files/SPICKER_data/spicker.data1");
	
	///@brief using Rosetta stage1 and stage2 for populatin initiolization
	bSkipStage1_ = false;	bSkipStage2_ = false;	bSkipStage3_ = true;	bSkipStage4_ = true;
	
	score2 = score_stage3a_;
	FragAssem_9 = brute_move_large_;
	FragAssem_3 = brute_move_small_;
...
//	core::util::switch_to_residue_type_set(nativePose, chemical::CENTROID, true);
...
void ClassicAbinitio::set_cycles( Real increase_cycles ) {
	stage1_cycles_ = static_cast< int > (200000 * increase_cycles);
